...
from contrastive_learning.Models.Data import Contrastive_Dataset
...
encdeor_model_name = 'Common_2DCNN'
config_model_name = "Test"  # 模型名称
TRAIN_FUNC = "ETE" # ETE or MOCO
...
images_dir = r'c:\Users\85002\Desktop\TempDIR\ZY-01-Test\clip_test2' # 数据集
ck_pth = None
...
if __name__ == '__main__':  
    ...
    config_model_name = f'{config_model_name}_Emb{out_embedding}'
    ...
    dataset = Contrastive_Dataset(image_lists)
    model = TRAIN_DICT[TRAIN_FUNC](encoder_model_name=encdeor_model_name, out_embedding=out_embedding, in_shape=dataset.data_shape, K=K, m=m, T=T)  # 模型实例化
    ...
    dataloader = DataLoader(dataset, batch_size=batch, shuffle=True, pin_memory=True, num_workers=dataloader_num_workers,
                            persistent_workers=True, drop_last=True)  # 数据迭代器
