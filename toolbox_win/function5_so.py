...
def sample_optimize(input_tif, input_shp_dir, data_form="Spectral", embedding_csv=None, FUNC="LF",  ratio=0.2, n_splits=1000):
    """
    优化采样点，剔除异常点
    
    参数:
        input_tif: 参考栅格路径
        input_shp_dir: 输入点Shapefile目录
        data_form: 数据形式 "Spectral"或"Embedding"
        embedding_csv: 降维特征CSV路径（data_form为"Embedding"时需要）
        FUNC: 异常检测方法 "LF"或"MCCV_LF"
        ratio: 异常样本比例（默认0.2即20%）
        n_splits: MCCV的分割次数（仅当FUNC为"MCCV_LF"时有效）
    ...
    try:
        ...
        if os.path.isdir(input_shp_dir):
            ...
            for idx, input_shp in enumerate(point_shp_files):
                    ...
                    if data_form == "Spectral":
                        data = get_spectral_from_shp(input_tif, input_shp) # 获取所有点的光谱数据
                    else:
                        if embedding_csv is not None:
                            data = read_csv_to_matrix(embedding_csv)
                            mask = idx_list
            ...
            if data_form == "Spectral":
                data = get_spectral_from_shp(input_tif, input_shp_dir) # 获取所有点的光谱数据
            else:
                if embedding_csv is not None:
                    data = read_csv_to_matrix(embedding_csv)
                    mask = idx_list
        ...
        return True, f"采样点优化成功, 结果保存至: {output_dir}"
    except Exception as e:
        return False, f"采样点优化失败: {e}"
    
if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument("--input_tif", type=str, required=True)
    parser.add_argument("--input_shp_dir", type=str, required=True)
    parser.add_argument("--data_form", type=str, default="Spectral", choices=["Spectral", "Embedding"])
    parser.add_argument("--embedding_csv", type=str, default=None)
    parser.add_argument("--FUNC", type=str, default="LF", choices=["LF", "Mccv_LF"])
